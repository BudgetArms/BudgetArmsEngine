...
namespace bae
{
    ...
    public:
        ...
        [[nodiscard]] float GetFPS() const { return 1.f / m_DeltaTime; }

        [[nodiscard]] float GetSleepTime() const
        {
            constexpr auto delayFrames                   = std::chrono::duration<float>(1.f / m_MaxFPS);
            const std::chrono::duration<float> sleepTime = m_LastTime + delayFrames -
    ...
    private:
        ...
        static constexpr int m_MaxFPS{ 240 };
        static constexpr float m_FixedTimeStep{ 1.f / 60.f };
