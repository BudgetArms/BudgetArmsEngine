...
AStar::AStar(Graph* pGraph, const Heuristic hFunction)
    : m_pGraph(pGraph), m_HeuristicFunction(hFunction)
{
}
...
std::vector<GraphNode*> AStar::FindPath(GraphNode* const pStartNode, const GraphNode* const pDestinationNode) const
{
    ...
    while(!openList.empty())
    {
        ...
        for(auto& nextNodeConnection : m_pGraph->GetConnectionsFromNode(currentNodeRecord.pNode))
        {
            ...
            const float nextNodeGCost = currentNodeRecord.costSoFar + nextNodeConnection->GetCost();
